...
class PreintegrationExample(object):
    ...
    def defaultParams(g):
        ...
        params.setGyroscopeCovariance(
            kGyroSigma ** 2 * np.identity(3, np.float32))
        params.setAccelerometerCovariance(
            kAccelSigma ** 2 * np.identity(3, np.float32))
        params.setIntegrationCovariance(
            0.0000001 ** 2 * np.identity(3, np.float32))
        return params
    ...
    def run(self, T=1):
        df = pd.read_csv('~/Desktop/Data/straight_right.csv')
        for i in range(len(df)):
            measuredOmega = df.iloc[0][9:12].values
            measuredAcc = df.iloc[0][0:3].values
            if i % 10 == 0: # accumulate 25 values
                print(i, measuredOmega, measuredAcc)
                self.plotImu(i, measuredOmega, measuredAcc)
                self.plotGroundTruthPose(i)
                # Integrate measurements for t seconds into a PIM (Preintegrated IMU Measurements)
                pim = self.runner.integrate(i, self.actualBias, True) 
                predictedNavState = self.runner.predict(pim, self.actualBias) # Predict given a PIM.
